...
# standard_feature_list = ['orit', 'rrna', 'inc-fac', 'circular']
...
def create_hdf5_datasets(directory, plasmid_classes_csv, chromosome_classes_csv, hdf5_path=None, test_fraction=0.2, batches_per_iteration=1, plasmid_batch_limit=None, chromosome_batch_limit=None):
    ...
    if(plas_limit == None):
        plas_limit = len(batchFiles[1])
    if (chrom_limit == None):
        chrom_limit = len(batchFiles[0])
    _logToFilewithPrint(
        f'{len(batchFiles[1])} batches of plasmids and {len(batchFiles[0])} batches of chromosomes found')
    if(plas_limit != len(batchFiles[1]) or chrom_limit != len(batchFiles[0])):
        _logToFilewithPrint(
    ...
    plasmid_labels_df = pd.read_csv(
        directory + '/plasmid/seq_target.csv', names=['batch', 'id', 'seq_ID'])
    plasmid_labels_df = plasmid_labels_df.drop_duplicates()
    if (plas_limit == len(batchFiles[1])):
    ...
    chromosome_labels_df = pd.read_csv(
        directory + '/chromosome/seq_target.csv', names=['batch', 'id', 'seq_ID', 'dom_seq_ID'])
    chromosome_labels_df = chromosome_labels_df.drop_duplicates()
    if (chrom_limit == len(batchFiles[0])):
...
def _checkFileStructure(directory, hdf5_path, plasmid_classes_csv, chromosome_classes_csv):
    ...
    for category_name in sub_directory_array:
        if (category_name == "extra-plasmid"):
            continue
        if(not os.path.exists(directory+"/"+category_name+"/seq_target.csv")):
